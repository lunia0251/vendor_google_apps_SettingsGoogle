...
class MaterialShapeQrFragment : Fragment() {
    ...
    companion object {
        ...
        fun createQrCode(content: String, ecLevel: ErrorCorrectionLevel): QRCode {
            ...
            return Encoder.encode(content, ecLevel, hints)
        }
    ...
    private var foregroundColorPrimary: Int = Color.GREEN
    private var foregroundColorSecondary: Int = Color.BLUE
    private var foregroundColorAccent: Int = Color.RED
    private var mainForegroundColorArray: Array<Int> =
        arrayOf(foregroundColorPrimary, foregroundColorSecondary)

    private var backgroundShapeColor: Int = Color.RED
    private var backgroundDotColor1: Int = Color.RED
    private var backgroundDotColor2: Int = Color.RED
    private var backgroundDotColor3: Int = Color.RED
    ...
    private fun loadVectorDrawable(context: Context, @DrawableRes resId: Int): VectorDrawable {
        return ContextCompat.getDrawable(context, resId) as VectorDrawable
    }
    ...
    override fun onCreateView(
        ...
        ViewCompat.setAccessibilityDelegate(frameLayoutView, qrCodeAccessibilityDelegate())
        ...
        val contentDescription = getString(com.android.settings.R.string.qr_code_content_description)
    ...
    private fun onQrCodeClicked() {
        ...
        val bitmap = QrCodeGenerator.encodeQrCode(content, pixelSize)
        qrCodeViewForStandard.setImageBitmap(bitmap)
    }

    private fun qrCodeAccessibilityDelegate(): AccessibilityDelegateCompat {
    ...
    fun updateQrCodeContent(
        qrCodeContent: String,
        qrCodeEcLevel: ErrorCorrectionLevel = ErrorCorrectionLevel.L,
    ) {
    ...
    private fun createMaterialShapeQRCode(content: String, ecLevel: ErrorCorrectionLevel) {
        ...
        val rawModuleSize =
            ceil(minOf(shortestScreenSide, 1200).toFloat() / qrcodeLineCount).toInt()
    ...
    private fun isForeground(col: Int, row: Int): Boolean {
        return (qrcode.matrix.get(col, row).toInt() and 15) == 1
    }
            ...
            for (col in 0 until qrcodeLineCount) {
                if (!hasCreated[col][row] && !isForeground(col, row)) {
                    createRendererForShape(col, row, 1, 1, shapeList, randomSquare(1)) {
                        ...
                        it.setStartDelay(83L + (1250f * ratio).toLong())
                        it.setSkipStartProgress(0.3f)
                        val paint =
                            Paint().apply {
                                colorFilter =
    ...
    private fun loadDynamicColors(context: Context) {
        ...
        backgroundDotColor1 =
            ColorUtils.setAlphaComponent(context.getColor(R.color.wifi_qr_primary_background), 38)
        backgroundDotColor2 =
            ColorUtils.setAlphaComponent(context.getColor(R.color.wifi_qr_secondary_background), 25)
        backgroundDotColor3 =
            ColorUtils.setAlphaComponent(context.getColor(R.color.wifi_qr_tertiary_background), 76)
